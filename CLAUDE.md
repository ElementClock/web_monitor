...
- **Tests / lint**: no formal test framework; regression tests live in [web/tests/](web/tests/) as self-contained scripts. Run them with `cd web/tests && ../../venv/Scripts/python.exe test_reassembly.py` (the venv is at the repo root, next to `web/`). Add new `test_*` functions there when touching frame handling. No linter configured.
...
- **SerialWindDataReader** [serial_reader.py](web/modules/serial_reader.py) — one instance per connected port; composes the pipeline (communicator + parser + storage). Defines `HEX_COMMAND` (a fixed 10-byte command sent to the device to request data). Each reader runs **three daemon threads** per port:
  1. *reconnect loop* — connects (retrying every 5s, exponential backoff to 30s on failure) and (re)starts the read thread;
  2. *command timer loop* — re-sends `HEX_COMMAND` every 1s (independent of data reads; exponential backoff to 10s on send failure);
  3. *read loop* — reads raw chunks, buffers and reassembles frames, parses, stores, writes CSV, and calls the data callback.
- **Frame reassembly & robustness** — `read_line()` returns raw chunks **without stripping newlines**; the read loop appends to `data_buffer` and `_reassemble_frames()` (under `_buffer_lock`) extracts complete frames. Two delimiting schemes are supported: newline-delimited (`\n`/`\r`) and `#...#`-delimited. **Fragmented/split frames (network congestion) simply wait in the buffer until their closing delimiter arrives** — no longer mis-parsed and dropped. Guards: `MAX_FRAME_LENGTH` (4000) caps a single frame; a frame with no closing `#` that exceeds it is truncated; buffer content with no delimiter at all is cleared when it exceeds the cap. `_handle_frame` centralizes parse → store → write → push. Robustness stats (`read_chunks`, `reassembled_frames`, `discarded_frames`, `buffer_truncations`, `last_reassembly`) are tracked per reader and exposed via `/api/status` as `robustness_stats`.
...
- Buffer safety rails exist on purpose: frame reassembly is capped by `MAX_FRAME_LENGTH` per frame with truncation fallbacks, and single `read()` calls cap `in_waiting` at 4096 to prevent OOM (see `P1-4`).
